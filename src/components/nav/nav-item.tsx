...
export default function NavigationIconLink(
    { isExpanded, className, href, label, icon, onClick, exact = false }:
        { isExpanded: boolean, className?: string, href: string, label: string, icon: React.ReactNode, onClick?: () => void, exact?: boolean }
) {
    ...
    const classes = [
        ...
        'hover:bg-brand-50',
        `${isActive ? 'bg-brand-100 text-brand-900 font-semibold' : ''}`,
        'group/item',
    ]
